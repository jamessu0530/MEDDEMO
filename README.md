...
- **「首次」標籤**：確認頁與回寫結果頁的競品旁邊，標出這家客戶以前確認過的拜訪從沒提過的競品。先後用拜訪編號比，不用拜訪時間：假資料的日期排在決賽日之前，展示時真的錄的拜訪卻是現在的時間。
- **風險分與主管通報**（原型：「競品已加入風險分，主管同步收到通報」）：
  - 確認送出時提到競品或客訴，就通報這家客戶轄區的主管。這兩項是客戶可能被搶走的直接訊號；意向、承諾是好消息或例行事項，不通報。
  - 風險分不另外發明權重，就是客戶檔案「進門前三分鐘」已經在判斷的五件壞事符合幾件（0～5）：進貨間隔拉長、答應客戶的事逾期、近 90 天提到競品、近 90 天客訴、帳款超過 60 天。合約快到期不算，那是行事曆上的事，不是客戶的行為出了問題。
  - 回寫結果頁寫「提到競品御松田，這家目前 4/5 項風險，已通報主管陳建宏」並列出項目；主管端多一個「風險通報」分頁，有未讀數，按「知道了」標已讀。分數記的是通報當下的判斷，之後不回頭改。
- **錄音時即時顯示文字（FR-4.2）**：
...
- **修改帳號**（照 flutterproject4）：帳號設定可以改名字、改密碼。
  - 名字規則照它的暱稱：2～32 字、擋不當用字。不照抄的兩處：那邊是遊戲排行榜 ID 所以不能重複、不能有空白；這裡是業務姓名，同名很正常，英文姓名要有空白（頭尾空白去掉、連續空白併成一個）。
  - **只有自己開的帳號能改名字**。公司的八個帳號名字是公司資料；而且自己開的帳號看的都是「林昱辰的客戶」，公司帳號被改名，所有人畫面上的示範業務都會跟著變。
...
### Google、GitHub、Facebook 登入

- **任何人都能用第三方帳號登入**，第一次登入自動開一個帳號（9/16 決定：決賽評審用自己的 Google 就能直接試）。
  - 自動開的帳號**一律是業務**，不會變主管；工號是 `X` 開頭，跟公司帳號（`U01`、`M01`）分得出來。
  - 自己名下沒有客戶，**今日路線與客戶看的是示範業務林昱辰（U01）的資料**，畫面上會標明。錄音送出的拜訪也會寫進林昱辰的客戶，多位評審同時試用時彼此看得到。
  - 沒有密碼，帳號設定不出現改密碼。如果第三方是它唯一的登入方式，不能解除綁定（解除了就永遠進不來）。
...
- **已經登入時的憑證錯誤回 400 不是 401**（改密碼時目前密碼打錯、綁定時第三方驗證失敗）：前端看到 401 會當成登入過期把人登出。
- 任何人都能開帳號，所以「每個帳號每小時」的用量上限擋不住有很多個 Google 帳號的人；擋住花費的是「全系統每天」那一層。

## 資料權限：只看得到自己的客戶

原型登入頁寫「登入後只會看到自己負責的客戶」、問答寫「只查得到自己轄區」。

- 業務只看得到自己負責的客戶；主管只看得到自己轄區（北區主管看北區兩位業務的 100 家）；第三方登入開的帳號看示範業務林昱辰的。規則在 `backend/app/services/scope.py`。
- 客戶清單、客戶檔案、談判卡、錄音上傳、拜訪紀錄都照這個範圍。看不到的客戶一律回 404，跟不存在一樣，不透露那個編號是別人的客戶。
- 轉給主管的提問：業務只看得到自己轉出去的，主管只看得到自己轄區轉來的。
- **數字查詢的過濾做在資料庫**：SQL 是模型寫的，靠提示叫它「只查自己的」擋不住。四個語意層 View 都用 `app_in_scope()` 過濾，範圍由 `sql_executor` 在每次查詢的交易裡設定。
  - 模型的 SQL 出現 `set_config`／`current_setting` 直接拒絕，但這只是讓錯誤訊息好懂：`U&"set\005fconfig"` 這種 Unicode 跳脫寫法擋不住文字檢查。**真正的保證是資料庫收回了唯讀角色呼叫 `set_config` 的權限**（`semantic_layer.sql`），測試裡兩種繞法都試過。
- 語音問答與錄音時的即時文字也要登入才發 Gemini 臨時金鑰（會花錢）。
...
- **商機**：模型與逾期承諾排出來的全是壞消息，業務會錯過正在變好的客戶。路線裡沒有商機時，把分數最高的一家商機換進來（替掉分數最低、不是硬排的那一站），最多一家。兩種商機：
  - 開了報價草稿（有下單意向），但之後這家還沒進過那個品項：報價還沒成交，值得去追。
  - 單次進貨金額比之前多兩成以上。兩成跟「進貨間隔拉長兩成」同一個幅度；假資料裡刻意設計的三家診所多了 29～33%，其他客戶的變動最多 8%。成長最多的是慢性處方時，提醒帶學名藥比價表（內部文件 17）。
  - 沒用「品類進貨成長兩成」：查過資料，那樣有一到兩成的客戶會亮，全是進貨剛好落在不同月份的雜訊。
...
- **開報價**（原型客戶檔案的按鈕）：不必先有一次拜訪，直接開 SAP 報價草稿。列這家近半年進過的品項與給這家的供貨價；**數量預設都是 0**，點「填入常進數量」才列入——報價通常只有一兩項，全部預填的話直接按送出就是一張十幾項的報價寫進 SAP。單號 `Q` 開頭，出現在待處理事項。模擬 SAP 停機時開不成。
...
- 刻意設計的「慢箋成長」：杏林診所 · 大安、明倫家醫科診所 · 北屯、光明家醫科診所 · 三民（三區各一家），慢性處方從 130 天前開始每次多進五成，是今日路線「商機」的驗證案例。數量在亂數抽完之後才放大，其他客戶的交易一筆都不變。
...
backend/app/services/risk.py        風險分、主管通報、競品「首次」
