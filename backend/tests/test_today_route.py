...
def route(client, user_id="U01", **feedback):
    body = {"user_id": user_id}
    if feedback:
        body["feedback"] = feedback
    response = client.post("/api/route/today", json=body)
    assert response.status_code == 200
    return response.json()


def test_reps_are_sales_only(client):
    reps = client.get("/api/reps").json()
    assert [r["id"] for r in reps] == ["U01", "U02", "U03", "U04", "U05"]
    assert all(r["name"] and r["region"] for r in reps)


def test_route_lists_five_stops_in_time_order(client):
    data = route(client)
...
def test_every_rep_gets_a_route_of_their_own_customers(client, engine):
    seen = set()
    for rep in client.get("/api/reps").json():
        stops = route(client, rep["id"])["stops"]
...
def test_urgent_card_explains_the_first_stop(client):
    data = route(client)
    urgent = data["urgent"]
...
def test_snoozing_drops_the_customer_from_today(client):
    first = route(client)["stops"][0]
    after = route(client, snoozed=[{"customer_id": first["customer_id"], "until": "2026-10-31"}])
...
def test_snooze_expires(client):
    first = route(client)["stops"][0]
    after = route(client, snoozed=[{"customer_id": first["customer_id"], "until": "2026-10-28"}])
    assert after["stops"][0]["customer_id"] == first["customer_id"]


def test_pinning_moves_a_customer_to_the_first_stop(client):
    stops = route(client)["stops"]
    last = stops[-1]
    after = route(client, pinned=[last["customer_id"]])
    assert after["stops"][0]["customer_id"] == last["customer_id"]
    assert after["stops"][0]["status"] == "next"


def test_marking_a_signal_wrong_pushes_that_kind_down(client):
    stops = route(client)["stops"]
    signal = stops[0]["signal"]
    after = route(client, signal_weights={signal: -5})["stops"]
    # 分數被壓一半，同類提醒不會再排第一站（除非整條路線都是同一類）
    assert after[0]["signal"] != signal or len({s["signal"] for s in stops}) == 1


def test_overdue_commitment_beats_the_model(client, engine):
    data = route(client)
...
def test_finished_visits_count_as_done(client, engine):
    with Session(engine) as session:
        stop = route(client)["stops"][1]
...
def test_unknown_or_manager_user_is_rejected(client):
    assert client.post("/api/route/today", json={"user_id": "U99"}).status_code == 404
    assert client.post("/api/route/today", json={"user_id": "M01"}).status_code == 404


def test_model_file_matches_the_features_in_code(client):
