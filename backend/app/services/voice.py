...
- search_knowledge：公司內部規定與作業文件（退貨、報價與折扣權限、帳款、陳列、客訴處理）。內部文件找不到時，可能改查網路上的公開資料，這時結果的 route 是 web。

規則：
1. 跟工作有關的問題一定要先呼叫工具，不能憑自己的知識、猜測或常識回答，也不能自己編數字。
2. 工具的結果怎麼說就怎麼講。結果是查無依據或查不完整，就照實說，並提醒業務畫面上可以轉給主管；結果的 reason 是 medical 時不要提轉主管，照結果請業務詢問醫師或藥師。不要改用常識補答。
3. 先講結論再講關鍵數字，一次不超過三句話。回答規定時順帶講出處的文件名稱；完整的表格和原文請業務看畫面上的卡片。結果的 route 是 web 時，第一句先說「公司文件裡沒有，以下是網路上的公開資料」，出處講網站名稱，不要說成公司的規定。
...
TOOLS = (
    ...
    Tool(
        "search_knowledge",
        "knowledge",
        "查公司內部規定與作業文件：退貨、報價與折扣權限、帳款、陳列與檔期、客訴處理等。"
        "內部文件查不到時可能改查網路公開資料（結果的 route 是 web），都查不到才回報查無依據。"
        "用藥、劑量、療效這類醫療問題，和只有公司內部才有答案的問題，內部文件查不到也不會上網"
        "（結果的 reason 是 medical 或 internal）。",
    ),
)
