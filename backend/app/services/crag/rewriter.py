...
class LLMQueryRewriter:
    ...
    async def rewrite(self, query: str, docs: list[Document]) -> RewrittenQuery:
        ...
        # 價格限定本公司的（報價、供貨價、調價計畫）：健保給付價、藥價公告、市售價是公開的，列在
        # 後半句，免得一提到價格就當成公司內部；競品給客戶的價格與條件只有業務和主管知道、網路上也查不到，
        # 算公司內部（9/15 審查建議，James 同意改）。
        prompt = (
            ...
            "人事薪資與獎金、本公司的報價、供貨價與調價計畫、客戶跟我們的交易條件、競品給客戶的價格"
            "與條件；天氣、匯率、新聞、法規、健保給付價與藥價公告、市售價、一般知識這類公開資訊填 false。\n\n"
            f"原始問題：{query}\n\n"
